...
async def _run_sql_safe(conn, query: str) -> None:
    try:
...
async def _run_schema_repair_migrations() -> None:
    async with engine.begin() as conn:
        if await _table_exists(conn, "users"):
            if not await _column_exists(conn, "users", "workspace_id"):
                await _run_sql_safe(conn, "ALTER TABLE users ADD COLUMN workspace_id INTEGER;")

        await _run_sql_safe(conn, "ALTER TABLE messages ADD COLUMN IF NOT EXISTS tokens INTEGER DEFAULT 0 NOT NULL;")
        await _run_sql_safe(conn, "ALTER TABLE conversations ALTER COLUMN title DROP NOT NULL;")

        # Normalize legacy table naming if older DB has user_memories.
        has_user_memory = await _table_exists(conn, "user_memory")
        has_user_memories = await _table_exists(conn, "user_memories")
        if not has_user_memory and has_user_memories:
            await _run_sql_safe(conn, "ALTER TABLE user_memories RENAME TO user_memory;")
        ...
        for table_name in [
            ...
            if await _table_exists(conn, table_name) and await _column_exists(conn, table_name, "created_at"):
                await _run_sql_safe(
                    conn,
        ...
        if await _table_exists(conn, "leads"):
            if await _column_exists(conn, "leads", "score"):
                await _run_sql_safe(conn, "ALTER TABLE leads ALTER COLUMN score SET DEFAULT 0;")
                await _run_sql_safe(conn, "UPDATE leads SET score = 0 WHERE score IS NULL;")
                await _run_sql_safe(conn, "ALTER TABLE leads ALTER COLUMN score SET NOT NULL;")
            if await _column_exists(conn, "leads", "status"):
                await _run_sql_safe(conn, "ALTER TABLE leads ALTER COLUMN status SET DEFAULT 'cold';")
                await _run_sql_safe(conn, "UPDATE leads SET status = 'cold' WHERE status IS NULL OR status = '';")
                await _run_sql_safe(conn, "ALTER TABLE leads ALTER COLUMN status SET NOT NULL;")
            if await _column_exists(conn, "leads", "name"):
                await _run_sql_safe(conn, "ALTER TABLE leads ALTER COLUMN name SET NOT NULL;")
