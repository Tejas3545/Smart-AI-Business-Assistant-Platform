    ...
    async with AsyncSession(engine) as session:
        ...
        try:
            if not workflow or not workflow.nodes:
                # If no workflow, just complete (or fail).
                task.status = "failed"
            ...
            if node_type == "condition":
                ...
                passed = False
                if operator == "==":
                    passed = (str(payload_val) == str(condition_value))
                elif operator == ">":
                    try:
                ...
                if passed:
                    ...
                    task.status = "completed" # Ends if condition fails
                    task.error_log = "Condition failed, halting workflow."
                    await session.commit()
                    return
                ...
                if provider == "webhook":
                    ...
                    if username and password and to_email:
                        ...
                        logger.warning("Missing email credentials or to_email")
                elif provider == "crm_hubspot":
                    api_key = creds.get("api_key")
                    await sync_crm_hubspot(api_key, task.payload)
                elif provider == "crm":
                    api_key = creds.get("api_key", "dummy")
...
async def automation_worker():
    """Background worker loops looking for pending tasks to handle."""
    while True:
        try:
            async with AsyncSession(engine) as session:
                ...
                if tasks:
                    await asyncio.gather(*(process_task(t.id) for t in tasks))
        except Exception as e:
            logger.error(f"Automation worker loop error: {e}")
